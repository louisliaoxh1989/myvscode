...
export class VSCodeMenu {
	...
	private setFileMenu(fileMenu: Electron.Menu): void {
        ...
        const openMy = new  MenuItem({ label: mnemonicLabel(nls.localize({ key: 'miNewWindow', comment: ['&& denotes a mnemonic'] }, "New &&Window")), click: () => this.openMyWindow() });
    ...
    public openMyWindow():void{

		let win = new BrowserWindow({
				alwaysOnTop: true,
				skipTaskbar: true,
				resizable: false,
				parent: this.windowsService.getFocusedWindow().win,
				modal: true,
				width: 450,
				height: 280,
				show: false,
			...
			win.loadURL(require.toUrl('vs/workbench/electron-browser/bootstrap/my.html'));
